...
//   work  - Workspace of size at least n-1 (n is always safe)
//
// The reflectors are generated by dlarfg and applied by dlarf, so the column
// norms are computed with dnrm2's scaled accumulation. Summing squares
// directly here would overflow to Inf on data with entries above ~1e154.
dgeqrf :: proc "contextless" (m: int, n: int, a: []f64, lda: int, tau: []f64, work: []f64) {
    k := min(m, n)

    for i in 0 ..< k {
        // The subcolumn below the diagonal, A(i+1:m-1, i), has stride lda.
        // At i == m-1 it is empty and dlarfg does not touch it.
        sub: []f64
        if i + 1 < m {
            sub = a[(i + 1) * lda + i:]
        }

        beta, ti := dlarfg(m - i, a[i * lda + i], sub, lda)
        tau[i] = ti

        // Apply H(i) = I - tau*v*v**T to A(i:m-1, i+1:n-1) from the left.
        // v is the subcolumn with an implicit 1 on the diagonal, so plant the
        // 1 in place for the duration of the update.
        if i + 1 < n && ti != 0.0 {
            a[i * lda + i] = 1.0
            dlarf(.Left, m - i, n - i - 1, a[i * lda + i:], lda, ti, a[i * lda + i + 1:], lda, work)
        }
...
dorgqr :: proc "contextless" (m: int, n: int, a: []f64, lda: int, tau: []f64, q: []f64, ldq: int, work: []f64) {
...
extract_r :: proc "contextless" (m: int, n: int, a: []f64, lda: int, r: []f64, ldr: int) {
    for i in 0 ..< m {
        for j in 0 ..< n {
            if j >= i {
                r[i * ldr + j] = a[i * lda + j]
